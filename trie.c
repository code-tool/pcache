...
stack_init(struct stack *s) {
    s->size = 256;
    s->fill = 0;
    s->stack = emalloc(s->size * sizeof(struct stack_node));
...
stack_grow(struct stack *s) {
    ...
    struct stack_node *resize = erealloc(s->stack, newsize);
    if (resize == NULL) {
        stack_free(s);
        return -1;
    }
...
trie_create(void) {
    ...
    trie *root = storage_malloc(sizeof(*root) + tail_size);
    if (root == NULL)
        return NULL;
    root->size = 255;
    root->nchildren = 0;
...
buffer_init(struct buffer *b, const char *prefix) {
    ...
    b->buffer = storage_malloc(b->size);
    if (b->buffer != NULL)
...
buffer_grow(struct buffer *b) {
    char *resize = erealloc(b->buffer, b->size * 2);
    if (resize == NULL) {
        buffer_free(b);
        return -1;
    }
